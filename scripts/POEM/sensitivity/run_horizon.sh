...
gpu_ids=${GPU_IDS:-0}
log_root="logs/sensitivity/seed_${seed}"
lock_root="$log_root/.locks"
. scripts/POEM/formal_config.sh

if [ "$seed" != 2021 ]; then
    echo "POEM sensitivity experiments are configured only for seed 2021" >&2
    exit 1
fi
...
# model_id phase_rank harmonics run_tag
variant_table() {
    cat <<'EOF'
Rank1 1 2 sensitivity_rank_1
Rank2 2 2 sensitivity_rank_2
Rank8 8 2 sensitivity_rank_8
Rank16 16 2 sensitivity_rank_16
Harmonics1 4 1 sensitivity_harmonics_1
Harmonics3 4 3 sensitivity_harmonics_3
Harmonics4 4 4 sensitivity_harmonics_4
Harmonics6 4 6 sensitivity_harmonics_6
EOF
}
...
is_complete() {
    ...
    local lr=${13} lradj=${14} enc_in=${15} phase_rank=${16} harmonics=${17}
    local run_tag=${18}
    local result_dir="results/sensitivity/seed_${seed}/${model_id}/${dataset}/pred_${pred_len}"

    [ -f "$result_dir/metrics.npy" ] && [ -f "$result_dir/settings.json" ] || return 1
    "$python_bin" - "$result_dir/settings.json" "$model_id" "$data_type" \
        "$data_path" "$d_model" "$period" "$layers" "$dropout" "$affine" \
        "$epochs" "$patience" "$batch" "$lr" "$lradj" "$enc_in" \
        "$phase_rank" "$harmonics" "$run_tag" "$pred_len" "$seed" <<'PY'
import json
import math
import sys
...
expected = {
    ...
    "phase_rank": int(sys.argv[16]),
    "harmonics": int(sys.argv[17]),
    "run_tag": sys.argv[18],
    "pred_len": int(sys.argv[19]),
    "seed": int(sys.argv[20]),
}
...
run_worker() {
    ...
    local model_id phase_rank harmonics run_tag
    local slot dataset data_type data_path formal_pred d_model period layers dropout
    local affine epochs patience batch lr lradj enc_in lock_fd log_dir log_path

    while read -r wanted_model wanted_dataset; do
        read -r model_id phase_rank harmonics run_tag \
        ...
        if is_complete "$model_id" "$dataset" "$data_type" "$data_path" \
            ...
            "$patience" "$batch" "$lr" "$lradj" "$enc_in" "$phase_rank" \
            "$harmonics" "$run_tag"; then
...
if ! flock -n "$run_lock_fd"; then
    echo "Another POEM sensitivity run for horizon ${pred_len} is active" >&2
    exit 1
fi
